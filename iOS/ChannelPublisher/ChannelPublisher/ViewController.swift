...
class ViewController: UIViewController {
    ...
    private var selectedQuality: PublishOption.Quality = .hd
    private var selectedBitrate: PublishOption.BitrateMode = .mbr

    /// Channel alias, to which the user will be publishing.
    ...
    override func viewDidLoad() {
        ...
        qualityButton.setTitle(selectedQuality.description, for: .normal)
        bitrateButton.setTitle(selectedBitrate.description, for: .normal)
    ...
    @IBAction func qualityButtonTapped(_ sender: UIButton) {
        showActionSheet(title: "Quality", values: PublishOption.Quality.allCases) {
            [weak self] selectedValue, description in

            sender.setTitle(description, for: .normal)
            self?.updateQuality(selectedValue)
        }
    }
    ...
    @IBAction func bitrateButtonTapped(_ sender: UIButton) {
        showActionSheet(title: "Bitrate Mode", values: PublishOption.BitrateMode.allCases) {
            [weak self] selectedValue, description in

            sender.setTitle(description, for: .normal)
            self?.updateBitrate(selectedValue)
        }
    }
    ...
    private func publish() {
        let capabilities: [String] = [selectedQuality.rawValue] + selectedBitrate.value
        ...
        channelPublisher.publish(channelAlias: channelAlias, userMediaStream: userMediaStream, capabilities: capabilities)
    }
    ...
    private func updateQuality(_ selectedValue: PublishOption.Quality) {
    ...
    private func updateBitrate(_ selectedValue: PublishOption.BitrateMode) {
        selectedBitrate = selectedValue
    }
}
